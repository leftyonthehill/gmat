...
truth_and_thrust_I_FM_wrapper = ForceModel("truth_and_thrust_I")
truth_and_thrust_I_FM_wrapper.setDynamics("truth")

"""truth_and_thrust_C_FM_wrapper = ForceModel("truth_and_thrust_C")
truth_and_thrust_C_FM_wrapper.setDynamics("truth")"""
...
truth_and_thrust_I_Prop_wrapper = Propagator("truth_and_thrust_I")
truth_and_thrust_I_Prop_wrapper.setIntegrator("truth")
truth_and_thrust_I_Prop_wrapper.setSat(truth_Sat_wrapper.getSat())

"""truth_and_thrust_C_Prop_wrapper = Propagator("truth_and_thrust_C")
truth_and_thrust_C_Prop_wrapper.setIntegrator("truth")
truth_and_thrust_C_Prop_wrapper.setSat(truth_Sat_wrapper.getSat())"""

gmat.Initialize()
"""thruster = truth_Sat_wrapper.getSat().GetRefObject(gmat.THRUSTER, truth_Sat_wrapper.thrusterNames[2])
thruster.SetField("IsFiring", True)
thrusterArray = "{" + ", ".join(truth_Sat_wrapper.thrusterNames) + "}"
truth_Sat_wrapper.getSat().SetField("Thrusters", thrusterArray)
truth_and_thrust_C_FM_wrapper.setThrust(truth_Sat_wrapper)
truth_and_thrust_C_Prop_wrapper.setFM(truth_and_thrust_C_FM_wrapper.getFM())
thruster.SetField("IsFiring", False)"""

thruster = truth_Sat_wrapper.getSat().GetRefObject(gmat.THRUSTER, truth_Sat_wrapper.thrusterNames[1])
thruster.SetField("IsFiring", True)
thrusterArray = "{" + ", ".join(truth_Sat_wrapper.thrusterNames) + "}"
truth_Sat_wrapper.getSat().SetField("Thrusters", thrusterArray)
truth_and_thrust_I_FM_wrapper.setThrust(truth_Sat_wrapper)
truth_and_thrust_I_Prop_wrapper.setFM(truth_and_thrust_I_FM_wrapper.getFM())
thruster.SetField("IsFiring", False)
...
truth_and_thrust_I_Prop_wrapper.prepareInternals()
# truth_and_thrust_C_Prop_wrapper.prepareInternals()
...
def enginesOn(axis:str):
    ...
    if axis=="R":
        ...
        pos = 1
        thruster = truth_Sat_wrapper.getSat().GetRefObject(gmat.THRUSTER, truth_Sat_wrapper.thrusterNames[pos])
        thruster.SetField("IsFiring", True)
        truth_Sat_wrapper.getSat().IsManeuvering(True)
        
        truth_and_thrust_I_Prop_wrapper.getPropagator().AddForce(truth_and_thrust_I_FM_wrapper.getBurnForce())
        truth_and_thrust_I_Prop_wrapper.getPropagator().AddPropObject(truth_Sat_wrapper.getSat())
        
        truth_and_thrust_I_Prop_wrapper.getPropagator().PrepareInternals()
        gator = truth_and_thrust_I_Prop_wrapper.getIntegrator()
        fm = truth_and_thrust_I_Prop_wrapper.getPropagator().GetODEModel()
    elif axis=="C":
        pos = 2
        """thruster = truth_Sat_wrapper.getSat().GetRefObject(gmat.THRUSTER, truth_Sat_wrapper.thrusterNames[pos])
        thruster.SetField("IsFiring", True)
        ...
        truth_and_thrust_C_Prop_wrapper.getPropagator().AddForce(truth_and_thrust_C_FM_wrapper.getBurnForce())
        truth_and_thrust_C_Prop_wrapper.getPropagator().AddPropObject(truth_Sat_wrapper.getSat())
        
        truth_and_thrust_C_Prop_wrapper.getPropagator().PrepareInternals()
        gator = truth_and_thrust_C_Prop_wrapper.getIntegrator()
        fm = truth_and_thrust_C_Prop_wrapper.getPropagator().GetODEModel()"""
    else:
        raise ValueError(f"Incorrect Thrust Axis. {axis} was entered but it can only be R, I, or C")    
    return gator, fm
...
state = "nominal"
gator_truth, fm_truth = enginesOff("I")
    ...
    match state:
        ...
        case "wait for I burn":
            if not(10 < truthCOE[-1] <= 170) and not(190 < truthCOE[-1] <= 350):
                ...
                gator_truth, fm_truth = enginesOn("I")
                ...
                print(f"Forces in {fm_truth.GetName()}:")
                for i in range(fm_truth.GetNumForces()):
                    f = fm_truth.GetForce(i)
                    print(" - ", f.GetName(), f.GetTypeName())
                print(gmat.Help("truth_and_thrust_I_Forces_Thrust"))
                print(f"Observed Two-body acceleration: [{temp2[0]:0.5e}, {temp2[1]:0.5e}, {temp2[2]:0.5e}]")
                """# exit(1)
        ...
        case "wait for C burn":
            ...
            gator_truth, fm_truth = enginesOn("C")
            state = "C burn"
            burn_duration = 0
        case "I burn":
            burn_duration += dt
